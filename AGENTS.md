...
- **Database init**: The Alembic migrations have dependency issues (migration 001 doesn't create all tables). Use `python scripts/init_db.py` to create all tables via `Base.metadata.create_all`, then `alembic stamp head` to mark migrations as applied. The `audit_logs` table is missing from `app/models/__init__.py` exports, so after `init_db.py` also run: `python -c "import asyncio; from app.core.database import engine, Base; from app.models.audit_log import AuditLog; asyncio.run(Base.metadata.create_all(engine))"` (this is idempotent).
